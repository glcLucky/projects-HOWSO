...
class Genetic_algorithm():
    """遗传算法类"""
    def __init__(self, bounds, fitfunc):
        """
        @bounds <list>: 各参数取值范围
        @fitfunc <func>: 适应度函数
        """
    ...
    # 需要根据决策变量的上下边界来确定
    def getEncodedLength(self, delta=0.0001):
        # 每个变量的编码长度
        lengths = []
    ...
    # 随机生成初始编码种群
    def getIntialPopulation(self, encodelength, populationSize):
        # 随机化初始种群为0
    ...
    # 染色体解码得到表现型的解
    def decodedChromosome(self, encodelength, chromosomes, delta=0.0001):
    ...
    def fit(self, max_iter=500, populationSize=10):
        """
        @max_iter <int>: 最大迭代次数
        @bounds <lst of lst>: 各变量边界值 [[], [], []]
        @fitfunc <function>: 适应度函数
        """
        # 每次迭代得到的最优解
        optimalSolutions = []
        optimalValues = []
        ...
        for iteration in range(max_iter):
            ...
            # 种群解码
            decoded = self.decodedChromosome(lengthEncode, chromosomesEncoded)
            # 得到个体适应度值和个体的累积概率
            evalvalues, cum_proba = self.getFitnessValue(decoded)
            # 选择新的种群
            ...
            final_decoded = self.decodedChromosome(
                lengthEncode, mutationpopulation)
            # 适应度评价
