...
# The data structure of pop is composed of several individuals which has
# the form like that:

#         {'Gene':a object of class Gene, 'fitness': 1.02(for example)}
# Representation of Gene is a list: [b s0 u0 sita0 s1 u1 sita1 s2 u2
# sita2]
...
# geneinfo.append(random.uniform(self.bound[0][pos],
# self.bound[1][pos]))#initialise popluation

#             fitness = evaluate(geneinfo)#evaluate each chromosome
# pop.append({'Gene':Gene(data = geneinfo), 'fitness':fitness})#store the
# chromosome and its fitness

#         self.pop = pop
# self.bestindividual = self.selectBest(self.pop)#store the best
...
# sum_fits = sum(1/ind['fitness'] for ind in individuals) #sum up the
# 1/fitness of the whole pop
...
# geninfo2 = offspring[1]['Gene'].data#Gene's data of second offspring
# chosen from the selected pop
...
# pos = random.randrange(1,dim)#chose a position in crossoff to perform
# mutation.
...
# fit_crossoff = evaluate(self.xydata, crossoff.data)# Evaluate the
# individuals
...
class Genetic_algorithm():
    ...
    def __init__(self, bounds, fitfunc, decimal=0.01, populationSize=10, mutprob=0.01, crossprob=0.8, max_iter=500):
        """
        @bounds <list of list>: 上下限列表 [[low, high], ..., [low, high]]
        @fitfunc <func>: 适应度函数
        @res_file_path <str>: 遗传算法中间迭代结果存储文件路径
        @decimal <float>: 精度  # 精度越小 收敛速度越慢
        @populationSize <int>: 种群规模  种群规模越大 收敛速度越慢
        @mutprob <float>: 变异概率 默认是0.01  变异概率越大 收敛速度越慢
        @crossprob <float>: 交叉概率 默认为0.8  交叉概率越大 收敛速度越慢
        """
    ...
    def getEncodedLength(self):
        ...
        for i in self.bounds:
            ...
            upper = i[1]
            # lamnda 代表匿名函数f(x)=0,50代表搜索的初始解
            res = fsolve(lambda x: ((upper - lower) * 1 /
                                    self.decimal) - 2 ** x - 1, 50)
            length = int(np.floor(res[0]))
            lengths.append(length)
        return lengths

    def getIntialPopulation(self, encodelength, populationSize=10):
        """
        根据种群规模和长度随机生成初始编码种群
        @encodelength <list>: 每个变量编码所需要的长度列表
        @populationSize <int>: 种群规模
        """
        chromosomes = np.zeros(
            (populationSize, sum(encodelength)), dtype=np.uint8)
    ...
    def getFitnessValue(self, chromosomesEncoded):
        """
        根据解码后的种群计算种群内各个体的适应度值
        @chromosomesEncoded <np.array> 群体 二维数组 必须是解码后的值
        """
        # 计算当前群体中每个个体的适应度值
        fitnees_value = self.fitfunc(chromosomesEncoded)  # 适应度值
    ...
    def selectNewPopulation(self, chromosomesEncoded, enter_prob):
        """
        轮盘赌选择
        - 根据适应度计算该种群每个个体的入选概率
    ...
    def crossover(self, chromosomesEncoded):
        """
        ...
        while len(index) > 0:
            ...
            crossoverPoint = crossoverPoint[0]
            # one-single-point crossover
            updatepopulation[a, 0:crossoverPoint] = chromosomesEncoded[
                a, 0:crossoverPoint]
            updatepopulation[a, crossoverPoint:] = chromosomesEncoded[
                b, crossoverPoint:]
            updatepopulation[b, 0:crossoverPoint] = chromosomesEncoded[
                b, 0:crossoverPoint]
            updatepopulation[b, crossoverPoint:] = chromosomesEncoded[
                a, crossoverPoint:]
        return updatepopulation

    def mutation(self, chromosomesEncoded):
        """
        对交叉后的染色体进行变异
        @chromosomesEncoded <np.array>: 经交叉后得到的种群（编码后）
        @return: 经变异操作后的新种群
        """
    ...
    def fit(self, log=False):
        ...
        for i in range(self.max_iter):
            ...
            if i == 0:
                chromosomesEncoded = self.getIntialPopulation(
                    lengthEncode)  # 编码后的染色体组
                chromosomesDecoded = self.decodedChromosome(lengthEncode, chromosomesEncoded)  # 解码后的染色体组
            fitness_value, enter_prob = self.getFitnessValue(chromosomesDecoded)  # 获得适应度值及其入选概率
            newpopulation = self.selectNewPopulation(
                chromosomesEncoded, enter_prob)  # 选择
            crossoverpopulation = self.crossover(newpopulation)  # 交叉
            chromosomesEncoded = self.mutation(chromosomesEncoded)  # 变异
            chromosomesDecoded = self.decodedChromosome(encodelength=lengthEncode, chromosomes=chromosomesEncoded)
            params[i] = chromosomesDecoded[
                np.argmax(self.fitfunc(chromosomesDecoded))]
            result[i] = self.fitfunc(chromosomesDecoded).max()
