...
#    svc = svm.SVC(kernel='rbf', gamma=x[0], C=x[1])
#    svc.fit(iris.data, iris.target)
#    return svc.score(iris.data, iris.target)


def main(bounds, fitfunc, max_iter=500):
    """
    @max_iter <int>: 最大迭代次数
    ...
    for iteration in range(max_iter):
        ...
        # 适应度评价
        fitnessvalues, cum_individual_proba = getFitnessValue(
            fitfunc, final_decoded)
    ...
    print("当前迭代下最优适应度函数值：{}".format(optimalValue))
