...
wminmax_func = function(cellinfo, problem_cell) {
  ...
  ####################
  # Horizontal angle # 方位角允许调整的角度范围 正负60度
  ####################
  # min
  ...
  ##################
  # Vertical angle # 下倾角允许调整的角度范围 依赖机械下倾角
  ##################
...
##########################
cost_from_df_updated = function(df_updated, problem_cell) {
  ...
  index_pb = which(df_summarize$ECI0 %in% problem_cell)
  sum_cnt = df_summarize$sum_cnt
  alpha_cnt = ifelse(sum_cnt > 500, 1, sum_cnt/500)[index_pb] # 权重
  ...
  a1 = sum((alpha_cnt*weak_cover_rate)[weak_cover_rate >= 0.1], na.rm=TRUE) # 弱覆盖率大于10%部分的加权损失 权重W
  a2 = sum(10*((alpha_cnt*weak_cover_rate)[weak_cover_rate < 0.1])^2, na.rm=TRUE) # 弱覆盖率小于10%部分的加权损失 权重W^2
