...
cellinfo[, cellinfo_numeric_feats] = suppressWarnings(sapply(cellinfo[, cellinfo_numeric_feats], as.numeric))
...
wminmax<-wminmax %>% inner_join(xwminmax,by=c("eci" = "ci","type"="type"))
wminmax$wmin<-ifelse(wminmax$wmin<wminmax$xwmin,wminmax$xwmin,wminmax$wmin)
wminmax$wmax<-ifelse(wminmax$wmax>wminmax$xwmax,wminmax$xwmax,wminmax$wmax)
col_non<-which(colnames(wminmax) %in% c('xwmin','xwmax','ci'))
wminmax<-wminmax[,-c(col_non)]
# print(wminmax)
...
#################
#################
print(cost_new(rep(0, nrow(wminmax))))
print(est.ras$optim$bestmem)
# ## Best member = best parameters x = (x1, x2) found:
# cat(est.ras$optim$bestmem)
if(cost_new(rep(0, nrow(wminmax)))>cost_new(rounding_param(est.ras$optim$bestmem))){
