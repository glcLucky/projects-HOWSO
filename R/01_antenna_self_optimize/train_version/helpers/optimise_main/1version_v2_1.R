...
nb_neighbor_eci = 9 # earfcn不全为空的邻区个??? 这个数字先设置为所有邻??? 然后根据后面函数计算出来
...
# cellinfo_azimuth_feat = "方位???" # useful for delta azimuth
...
# # Features which are not used: "日期", "小区名称", "参考信号功???", "电下倾角"
###########################
cellinfo_eci_feat = "ci"
...
# Default value for "方位???" == cellinfo_azimuth_feat
...
# Change missing values caused by missing neighbor ECIs 处理那些因为缺失eci信息而缺失的???
#
...
# For i from 1 to 9:
# "邻区i的参考信号接收功???" --> receiving power of ith adjacent area
...
data_OTT_u<-data.table(data_OTT_u) # zwk 指纹???
...
# 如果selected_cell不为空则选取这些小区的工参信??? 否则就选取上述获得的可行小???
...
names(cellinfo)[which(names(cellinfo) == cellinfo_azimuth_feat)] = "方位???"
...
for(i in c(0:nb_neighbor_eci)) {
  ### 无邻???,有邻区但邻区 不在调优范围,室内ott经纬度与小区经纬度相???
  ...
  ### Horizontal angle 计算水平??? 用于计算方位???
  # Horizontal 包括 %二象???+360 第三四象限及Y轴负半轴+180
  ...
  # dis_m_u is only used to deduce vertical_angle dis_m_U是临时变??? 仅仅用来计算垂直???
  ...
  # 计算夹角，向下取整，由于方向图基本对称，可以使用对称的方式计算增益所以使用绝对???
  df[[paste0("delta_azimuth", i)]][idx] = 
    floor_diff(df[[paste0("方位???", i)]][idx], 
               df[[paste0("horizontal_angle", i)]][idx])
...
start.time = Sys.time()
print(start.time)
est.ras <- DEoptim(cost_new2, # function to optimize 第一个参数是待优化参??? 根据下面的lower和upper确定 这里即所有问题小区的两个角度
